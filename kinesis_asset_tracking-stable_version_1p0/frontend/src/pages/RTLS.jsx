...
import { Play, Pause, RotateCcw, Battery, Activity, Circle, Filter, X, Search, User } from 'lucide-react';
...
const RTLS = () => {
  ...
  // Stores the exact draw area of the floor plan image on the canvas
  // All anchors and tags use these bounds so they always render inside the image
  ...
  useEffect(() => {
    const savedPatients = localStorage.getItem('patientTagAssignments');
    if (savedPatients) {
      const assignments = JSON.parse(savedPatients);
      setPatientAssignments(assignments.filter(a => a.status === 'active'));
    }
    const savedEquipment = localStorage.getItem('equipmentAssignments');
    if (savedEquipment) {
      setEquipmentAssignments(JSON.parse(savedEquipment));
    }
  }, []);
  ...
  useEffect(() => {
    ...
    const connect = () => {
      ...
      ws.onmessage = (event) => {
        ...
        if (message.type === 'tag_update' && !playing) {
          setTags(() => {
            return [...message.data].sort((a, b) => {
              if (a.status === b.status) return a.device_id.localeCompare(b.device_id);
  ...
  // Load image once and calculate + store its draw bounds
  // Any map uploaded will auto-calculate correct bounds here
  useEffect(() => {
    if (floorPlan?.image) {
      const img = new Image();
      img.onload = () => {
  ...
  // Helper: convert tag/anchor coordinates to canvas pixel position using image bounds
  ...
  useEffect(() => {
    ...
    // Draw cached floor plan image using stored bounds
    ...
    // Draw anchors — use toCanvasPos so they always land inside the image
    ...
    // Draw tags — also use toCanvasPos via processTagClusters
  ...
  const handleAssetClick = (tag) => {
    ...
    if (tag.x && tag.y) toast.success(`Centered on ${tag.device_id}`);
  };
            ...
            <CardContent className="space-y-4">
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <Button variant="ghost" size="sm" onClick={() => setSelectedTags(new Set())} className="text-xs">Clear All</Button>
                </div>
              ...
              <ScrollArea className="h-[800px] pr-2">
                <div className="space-y-2">
                  {getFilteredTags().map(tag => {
                    ...
                    const equipment = equipmentAssignments[tag.device_id];
                    return (
                      <div key={tag.device_id} onClick={() => handleAssetClick(tag)}
